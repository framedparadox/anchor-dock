...
/// <summary>
/// Toggles "launch DockGx at sign-in" via the per-user <c>Run</c> registry key. Per-user (HKCU)
...
public static class StartupService
{
    ...
    private const string ValueName = "DockGx";
    ...
    public static void SetEnabled(bool enabled)
    {
        try
        {
            ...
            if (enabled)
            {
                var exe = Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "DockGx.exe");
