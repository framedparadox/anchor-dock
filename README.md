...
- **Snap to any edge → hides where you left it** — drop the dock near a screen edge and it snaps
  ...
  another monitor** it still hides, but the dock never crosses onto the neighboring screen: it
  collapses to the **notch** on the monitor it is snapped to, and only that notch stays visible.
...
monitor you dropped it on. On an edge that borders another monitor it still hides, but only the
notch remains — on the snapped screen; the dock does not slide onto the neighbor. Or give each
screen its own: **Settings ▸ Docks ▸ Add dock**, then pick its monitor — each dock keeps its own
items and edge.
